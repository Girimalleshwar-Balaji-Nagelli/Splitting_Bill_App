...
class SplitByAmountFragment : Fragment() {

    private lateinit var recyclerView : RecyclerView
    private lateinit var adapter: RecyclerViewAdapter_Amount
    ...
    override fun onCreateView(
        ...
        val view =  inflater.inflate(R.layout.fragment_split_by_amount, container, false)

        var totalAmount = 0.0

        recyclerView = view.findViewById(R.id.recview_for_split_by_amount)
        textView = view.findViewById(R.id.total_amount)
//        itemList = List(5) {index -> anotherItem("Item #$index") }

        userList = emptyList()

        adapter = RecyclerViewAdapter_Amount(userList)
